...
def train(params):
    ...
    train_dataset = ScanNetObjPairScore('train')
    val_dataset = ScanNetObjPairScore('train')
    ...
    # model
    model = PCNScore(num_dense=16384, latent_dim=1024, grid_size=4).to(params.device)
    ...
    for epoch in range(1, params.epochs + 1):
        ...
        # training
        model.train()
        for i, (p, c, score) in enumerate(train_dataloader):
            ...
            loss = loss1 + alpha * loss2 + lambda_score * loss_score
            ...
            if (i + 1) % step == 0:
                log(log_fd, "Training Epoch [{:03d}/{:03d}] - Iteration [{:03d}/{:03d}]: coarse loss = {:.6f}, dense l1 cd = {:.6f}, score mse {:.6f} total loss = {:.6f}"
                    .format(epoch, params.epochs, i + 1, len(train_dataloader), loss1.item() * 1e3, loss2.item() * 1e3, loss_score.item()* 1e3, loss.item() * 1e3))
