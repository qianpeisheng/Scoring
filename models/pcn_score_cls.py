...
class PCNScoreCls(nn.Module):
    ...
    def __init__(self, num_dense=16384, latent_dim=1024, grid_size=4):
        ...
        # add a score prediction layer where the input size is (B, 1024), and the output size is 1. Just one layer.
        ...
        # add a classification head of 40 classes
    ...
    def forward(self, xyz):
        ...
        score = self.score_layer(feature_global) # (B, 1024) -> (B, 512) -> (B, 1)

        classification = self.classification_layer(feature_global) # (B, 1024) -> (B, 512) -> (B, 40)
        ...
        return coarse.contiguous(), fine.transpose(1, 2).contiguous(), score, classification
